...
FOLLOWFACTOR = {TIMES, DIV, MOD, AND, OR, PLUS, MINUS, EQ, NE, LT, LE, GT, GE,
                ...
                BAR, END}
FIRSTEXPRESSION = {PLUS, MINUS, IDENT, NUMBER, LPAREN, NOT, TILDE, BAR}
...
# extend factor() to parse unary similarly to not. If the operand is not Int, an error not integer is
# generated, if the operand is a constant, the expression is evaluated ("constant folding") by using
#Python’s , otherwise the genUnaryOp of the code generator is called.
...
#extend term() to parse binary & similarly to and, *. If not both operands are Int, an error bad
#type is generated, if both operands are constant, the expression is evaluated by using Python’s
#&, otherwise genBinaryOp of the code generator is called.
...
#extend simpleExpression() to parse binary | similarly to or, +. If not both operands are Int, an
#error ‘bad type‘ is generated, if both operands are constant, the expression is evaluated by using
#Python’s |, otherwise genBinaryOp of the code generator is called. - extend the first and follow
#sets of factor and expression accordingly.
